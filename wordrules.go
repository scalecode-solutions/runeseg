...
func wbTransitions(state, prop int) (newState int, wordBreak bool, rule int) {
	...
	case wbAny | prALetterExtPict<<32:
		return wbALetter, true, 9990 // ALetter_ExtPict treated as ALetter
	case wbAny | prHebrewLetter<<32:
		return wbHebrewLetter, true, 9990
	case wbALetter | prALetter<<32:
		return wbALetter, false, 50
	case wbALetter | prALetterExtPict<<32:
		return wbALetter, false, 50 // ALetter_ExtPict treated as ALetter
	case wbALetter | prHebrewLetter<<32:
		return wbHebrewLetter, false, 50
	case wbHebrewLetter | prALetter<<32:
		return wbALetter, false, 50
	case wbHebrewLetter | prALetterExtPict<<32:
		return wbALetter, false, 50 // ALetter_ExtPict treated as ALetter
	...
	case wbWB7 | prALetterExtPict<<32:
		return wbALetter, false, 70 // ALetter_ExtPict treated as ALetter
	...
	case wbHebrewLetter | prNumeric<<32:
		return wbNumeric, false, 90
	// Note: wbALetterExtPict doesn't need separate entry since it transitions to wbALetter

	// WB10.
	case wbNumeric | prALetter<<32:
		return wbALetter, false, 100
	case wbNumeric | prALetterExtPict<<32:
		return wbALetter, false, 100 // ALetter_ExtPict treated as ALetter
	...
	case wbExtendNumLet | prExtendNumLet<<32:
		return wbExtendNumLet, false, 131
	// Note: wbALetterExtPict transitions to wbALetter, so it's handled above

	// WB13b.
	case wbExtendNumLet | prALetter<<32:
		return wbALetter, false, 132
	case wbExtendNumLet | prALetterExtPict<<32:
		return wbALetter, false, 132 // ALetter_ExtPict treated as ALetter
...
func transitionWordBreakState(state int, r rune, b []byte, str string) (newState int, wordBreak bool) {
	...
	if nextProperty == prZWJ {
		...
		return state, false
	} else if (nextProperty == prExtendedPictographic || nextProperty == prALetterExtPict) && state >= 0 && state&wbZWJBit != 0 {
		// WB3c / WB3.3: ZWJ × (Extended_Pictographic | ALetter_ExtPict)
		if nextProperty == prALetterExtPict {
			return wbALetter, false // ALetter_ExtPict transitions to ALetter state
		}
		return wbAny, false
	}
	if state >= 0 {
	...
	if rule > 60 &&
		...
		(farProperty == prALetter || farProperty == prHebrewLetter || farProperty == prALetterExtPict) {
		return wbWB7, false
	}
