...
const (
	propertyURL = `https://www.unicode.org/Public/17.0.0/ucd/%s.txt`
	emojiURL    = `https://unicode.org/Public/17.0.0/ucd/emoji/emoji-data.txt`
)
...
func parse(propertyURL, emojiProperty string, includeGeneralCategory bool) (string, error) {
	...
	// Build a map of code points that already have properties from the main file.
	// This tracks the property name for each code point.
	existingProperties := make(map[int]string)
	for _, prop := range properties {
		from, _ := strconv.ParseInt(prop[0], 16, 64)
		to, _ := strconv.ParseInt(prop[1], 16, 64)
		for cp := from; cp <= to; cp++ {
			existingProperties[int(cp)] = prop[2]
		}
	}

	// Open the second URL (emoji data).
	if emojiProperty != "" {
		...
		for scanner.Scan() {
			...
			// For each code point in the range, check if it already has a property.
			// If it has ALetter, use the combined ALetter_ExtPict property.
			// If it has no property, use Extended_Pictographic.
			fromInt, _ := strconv.ParseInt(from, 16, 64)
			toInt, _ := strconv.ParseInt(to, 16, 64)

			// Track ranges of code points with the same effective property.
			var rangeStart int64 = -1
			var currentProp string
			for cp := fromInt; cp <= toInt; cp++ {
				existingProp := existingProperties[int(cp)]
				var effectiveProp string
				if existingProp == "ALetter" {
					// ALetter + Extended_Pictographic = ALetter_ExtPict
					effectiveProp = "ALetter_ExtPict"
				} else if existingProp != "" {
					// Other property exists (like Katakana, Numeric) - skip emoji property
					// for this code point to avoid breaking existing behavior
					effectiveProp = ""
				} else {
					// No existing property - use Extended_Pictographic
					effectiveProp = property
				}

				if effectiveProp == "" {
					// Close any open range
					if rangeStart != -1 {
						properties = append(properties, [4]string{
							fmt.Sprintf("%04X", rangeStart),
							fmt.Sprintf("%04X", cp-1),
							currentProp,
							comment,
						})
						rangeStart = -1
					}
				} else if effectiveProp != currentProp {
					// Different property - close old range and start new
					if rangeStart != -1 {
						properties = append(properties, [4]string{
							fmt.Sprintf("%04X", rangeStart),
							fmt.Sprintf("%04X", cp-1),
					...
					rangeStart = cp
					currentProp = effectiveProp
				}
				// else: same property, continue the range
			}
			// Close any remaining open range.
			if rangeStart != -1 {
				properties = append(properties, [4]string{
					fmt.Sprintf("%04X", rangeStart),
					fmt.Sprintf("%04X", toInt),
					currentProp,
					comment,
				})
			}
