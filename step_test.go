...
func TestStepBytesWord(t *testing.T) {
	for testNum, testCase := range wordBreakTestCases {
		// These test cases reveal an inconsistency in the Unicode rule set,
		// namely the handling of ZWJ within RI graphemes. (Grapheme rules will
		// restart the RI count, word rules will ignore the ZWJ.)
		// Test 1700 was for Unicode 15.0, tests 1820-1821 are for Unicode 17.0.
		if testNum == 1700 || testNum == 1820 || testNum == 1821 {
			continue
		}
		/*t.Logf(`Test case %d %q: Expecting %x, getting %x, code points %x"`,
		testNum,
...
func FuzzStepString(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, orig string) {
		...
		// https://github.com/scalecode-solutions/runeseg/issues
	})
}
